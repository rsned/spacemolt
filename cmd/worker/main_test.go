...
func TestBuildStatusAndKnownState(t *testing.T) {
	...
	got := buildStatus(st, "track_station", "t-1", "", statusFlags{Connected: true}, now)
...
func TestBuildStatusDockedOnlyAtStation(t *testing.T) {
	...
	if buildStatus(parked, "shuttle", "", "", statusFlags{Connected: true}, now).Docked {
	...
	if buildStatus(transit, "hauler", "", "", statusFlags{Connected: true}, now).Docked {
...
func TestBuildStatusPrefersSystemDisplayName(t *testing.T) {
	...
	got := buildStatus(st, "hauler", "", "", statusFlags{Connected: true}, time.Unix(1000, 0))
...
func TestBuildStatusCarriesDrained(t *testing.T) {
	...
	if got := buildStatus(st, "hauler", "", "", statusFlags{Drained: true, Connected: true}, time.Unix(1000, 0)); !got.Drained {
		t.Fatalf("Drained = false, want true")
	}
	if got := buildStatus(st, "hauler", "", "", statusFlags{Connected: true}, time.Unix(1000, 0)); got.Drained {
		t.Fatalf("Drained = true, want false")
	}
}

// buildStatus must report Disconnected=true exactly when the game connection is
// down, so the supervisor can leave a reconnecting worker to the reconnect gate
// instead of restarting it (a restart's fresh login deepens a per-IP block).
func TestBuildStatusCarriesDisconnected(t *testing.T) {
	st := &game.State{CurrentSystem: "sol"}
	if got := buildStatus(st, "hauler", "", "", statusFlags{}, time.Unix(1000, 0)); !got.Disconnected {
		t.Fatalf("Disconnected = false, want true when connected=false")
	}
	if got := buildStatus(st, "hauler", "", "", statusFlags{Connected: true}, time.Unix(1000, 0)); got.Disconnected {
		t.Fatalf("Disconnected = true, want false when connected=true")
	}
}
