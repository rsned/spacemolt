...
func TestBuildStatusAndKnownState(t *testing.T) {
	...
	got := buildStatus(st, "track_station", "t-1", "", false, true, now)
...
func TestBuildStatusDockedOnlyAtStation(t *testing.T) {
	...
	if buildStatus(parked, "shuttle", "", "", false, true, now).Docked {
	...
	if buildStatus(transit, "hauler", "", "", false, true, now).Docked {
...
func TestBuildStatusPrefersSystemDisplayName(t *testing.T) {
	...
	got := buildStatus(st, "hauler", "", "", false, true, time.Unix(1000, 0))
...
func TestBuildStatusCarriesDrained(t *testing.T) {
	...
	if got := buildStatus(st, "hauler", "", "", true, true, time.Unix(1000, 0)); !got.Drained {
		t.Fatalf("Drained = false, want true")
	}
	if got := buildStatus(st, "hauler", "", "", false, true, time.Unix(1000, 0)); got.Drained {
...
func TestBuildStatusCarriesDisconnected(t *testing.T) {
	st := &game.State{CurrentSystem: "sol"}
	if got := buildStatus(st, "hauler", "", "", false, false, time.Unix(1000, 0)); !got.Disconnected {
		t.Fatalf("Disconnected = false, want true when connected=false")
	}
	if got := buildStatus(st, "hauler", "", "", false, true, time.Unix(1000, 0)); got.Disconnected {
