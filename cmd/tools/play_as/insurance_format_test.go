...
func TestFormatGetInsuranceQuote_RendersQuote(t *testing.T) {
	raw := []byte(`{"message":"Insurance quote for your Census (fitted value: 33535 credits)",` +
		`"quote":{"coverage":30000,"premium":4200,"fitted_value":33535,"risk_score":0.42,"expires_in":50,` +
		`"factors":[` +
		`{"name":"Ship Condition","detail":"Hull at 92%","multiplier":1.05},` +
		`{"name":"Security","detail":"High-sec space","multiplier":0.8}` +
		`]}}`)
