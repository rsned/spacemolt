...
- **⭐🟢✅ 09-08 — COLD START after a ~2h15m hard stop: 9 fleets, 170 workers, 164 healthy, restarts=0, all on `389d3695`.** Followed `docs/COLD_START.md`; rebuilt every binary at the stop (overmind/worker were 37min behind HEAD, scanner/prune/secondment weeks behind). Haul gated correctly and claimed opps within seconds. **The 6 unhealthy = exactly the 6 non-`done` rescue-queue records** (assist-sol, assist-krynn, fighter-7, fighter-10, explorer-1, engineer-1), quarantined since 09-04..06 and left as-is. Runbook is stale on the fleet set. [[reference_cold_start_runbook_drift]] · [[reference_rescue_queue_blocks_launch]]
...
- **⭐ capture cadence retune — APPLIED 09-08** (128 agents hourly -> twice_daily). It had been recorded as applied on 08-30 but had NOT stuck; **dry-run the script at every fleet stop** rather than trusting the note. [[reference_capture_cadence_retune]]
