...
## Rolled from the Live board 2026-09-08 (resolved / superseded)
...
- **08-31 (night) — phantom-inventory relisting livelock KILLED `3bdc887d`** (single-form create_sell_order never updated the cargo cache; insufficient_items now completes the claim — 216 futile listings/10 workers during the power_cell wall). v0.573.2 push payloads absorbed `feac0e82`. Haul fleet redeployed onto the fixed worker. trader-10's 675 cells parked at 30,900 awaiting the wall's return. [[project_titanium_alloy_war_demand]]
