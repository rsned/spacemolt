...
- ⭐ [faction_withdraw_items target = self | faction](reference_faction_withdraw_pulls_personal_storage.md) — `self` is YOUR storage (silent trap, name implies otherwise); `faction` is the lockbox. Lockbox holds 38k liquid_hydrogen ≈ 19k fuel cells
