...
- ⭐🟢 [Live KB at v60; chain collapsed to a v59 baseline](reference_live_kb_schema_drift.md) — applied 09-08 15:34 via `bin/kb-migrate` (54 ms, fleet live); backup = `VACUUM INTO`, never shell `.backup`; seen_players SQLITE_BUSY ~6/min is CHRONIC. NEVER edit an applied migration
...
- ⭐🔴 [`connections` errs BOTH ways](reference_connections_phantom_edges.md) — 19 phantom edges (copied neighbour lists) shorten routes; wormhole lanes are absent entirely, so real one-way shortcuts are invisible
- ⭐🔴 [`poi_resources.remaining=0` is NOT depletion](reference_poi_resources_remaining_is_unreliable.md) — commerce_fields reads 0 while actively yielding all 5 resources; rank by `richness`
