...
- ⭐🔴 [NEVER restart a disconnected worker](reference_never_restart_a_disconnected_worker.md) — a restart cannot reconnect it, only the gate can; 70 workers behind one paced gate ALWAYS outlast a 30min grace. FIXED a35f94c5
- ⭐🔴 [False rescue records: max_fuel=0 = STATE-READ FAILURE](reference_stale_rescue_records_outlive_the_strand.md) — the reconnect WEDGE manufactures them with no block at all (haul 16→0 in 7h); 6-for-6 "real" strandees were docked+BROKE, 1.5k credits total. Never xargs a username
...
- ⭐ [faction_withdraw_items: target = SOURCE, destination is always CARGO](reference_faction_withdraw_pulls_personal_storage.md) — omit=lockbox, `self`=your own storage (trap); hull-bounded, so 38k liquid_hydrogen ≈ 20 Congregation loads. `pending` ack ≠ success
...
- ⭐ [send_gift ship_id transfers a hull FREE](reference_send_gift_ship_transfer.md) — remote send, but the ship STAYS PARKED; recipient must travel to it. Credit gifts need no co-location
- ⭐🔴 [commission_ship has TWO modes](reference_ship_commissioning_is_credits_only.md) — credits-only 38,603 vs provide-materials 5,939 labour+components; BUYING at ~13,300 beats credits-only; outerrim hulls need an Outer Rim yard
