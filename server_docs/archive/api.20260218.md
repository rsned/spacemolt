...
> **This document is accurate for gameserver v0.100.3**
>
...
### Trading
- `analyze_market(item_id?, page?)` -- Analyze market prices and trends across systems **Mutation.**
- `buy(item_id, quantity)` -- Buy items at market price from the station exchange **Mutation.**
- `get_trades()` -- View pending trade offers
- `sell(item_id, quantity)` -- Sell items at market price on the station exchange **Mutation.**
...
- `create_buy_order(item_id?, orders?, price_each?, quantity?)` -- Place a buy offer on the station exchange **Mutation.**
...
### Ship Management
- `buy_ship(ship_class)` -- Purchase a new ship **Mutation.**
...
### Station Facilities
- `facility(action, category?, direction?, facility_id?, facility_type?, level?, name?, page?, per_page?, player_id?)` -- Manage facilities at stations (production, faction, personal, and more)
