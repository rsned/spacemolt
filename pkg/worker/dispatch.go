...
func (d *WorkerDispatch) Run(ctx context.Context, tokens []string) error {
	...
	case "fit_drones":
		...
		// strip defaults FALSE: uninstalling is the exception, and a caller that
		// wants a module removed says so.
		strip := false
		if len(args) >= 7 {
			b, err := strconv.ParseBool(args[6])
			if err != nil {
				return fmt.Errorf("fit_drones: bad strip %q (want true/false)", args[6])
			}
			strip = b
		}
	...
	case "capture_action_log":
		...
		return err
	case "capture_tax":
		// Reads the NEXT levy a week ahead. An unpayable levy turns into an
	...
	case "capture_wildlife_attacks":
		...
		// the queue of battles we killed something in and never read the log of.
		return d.CaptureWildlifeAttacks(ctx, args)
	default:
