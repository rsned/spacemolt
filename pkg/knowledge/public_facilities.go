		...
		for _, r := range rows {
			if _, err := tx.ExecContext(ctx, `
				INSERT INTO public_facilities
					(station_id, facility_id, recipe_id, facility_name, category, level, rental_fee_per_run, owner_faction, public, details_json, last_seen_tick)
				VALUES (?,?,?,?,?,?,?,?,1,?,?)
...
func (kb *SQLiteKB) FacilitiesForRecipe(ctx context.Context, recipeID string) ([]PublicFacility, error) {
	rows, err := kb.db.QueryContext(ctx, `
		SELECT station_id, facility_id, recipe_id, facility_name, category, owner_faction, level, rental_fee_per_run, last_seen_tick
	...
	for rows.Next() {
		...
		if err := rows.Scan(&f.StationID, &f.FacilityID, &f.RecipeID, &f.FacilityName, &f.Category,
			&f.OwnerFaction, &f.Level, &f.RentalFeePerRun, &f.LastSeenTick); err != nil {
			return nil, err
		}
		out = append(out, f)
	}
