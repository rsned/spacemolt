...
- **Minimal dependencies.** Runs locally on Ubuntu with `go run ./server` + `npm run dev` (or a single binary serving the built frontend). No cloud services required.
...
│  └──────────┘  └───────────┘  └───────────────┘  │
│  ┌──────────┐  ┌───────────────────────────────┐  │
│  │ Database  │  │  Local LLM Client (future)   │  │
│  │ (future)  │  │  (HTTP to Ollama/vLLM/etc.)  │  │
│  └──────────┘  └───────────────────────────────┘  │
└────────────────────────┬─────────────────────────┘
                         │ WebSocket (wss://)
┌────────────────────────┴─────────────────────────┐
...
| **Go Backend Server** | Maintains persistent WebSocket connections to SpaceMolt per agent (one goroutine each), receives push state updates, relays to browser, proxies mutation commands. Future: database and local LLM integration. |
| **SpaceMolt WebSocket API** | Authoritative game server; persistent connection with real-time push notifications |

### 2.3 Why a Local Backend? Why Go?

The SpaceMolt WebSocket API (`wss://game.spacemolt.com/ws`) provides persistent connections with push notifications — no polling needed for core state. A Go backend:

- Manages one persistent WebSocket per agent, each in its own goroutine — Go's concurrency model makes 100+ simultaneous agent connections trivial with minimal resource overhead.
...
- Handles multiple agent sessions simultaneously (tested with up to 100 agents; spacing out connections prevents rate limit issues).
...
- Provides clean integration points for future additions: database storage via standard Go drivers (`pgx`, `go-sqlite3`), local LLM communication via HTTP client to Ollama/vLLM/etc.
- Consolidates backend work in Go, consistent with the existing SpaceMolt MCP server and market data infrastructure already being built in Go.
...
| Frontend build tool | **Vite** | Fast HMR, excellent TypeScript support |
| Frontend state | **Zustand** | Lightweight, works well with WebSocket-driven updates |
| Backend server | **Go** (`net/http` + `gorilla/websocket` or `nhooyr.io/websocket`) | Native concurrency via goroutines (one per agent connection), excellent WebSocket libraries, strong DB drivers, straightforward HTTP client for local LLM APIs. Consistent with existing Go-based SpaceMolt tooling (MCP server, market data). |
...
| Browser↔Backend | **WebSocket** (`ws://localhost:PORT`) | Go backend relays game state to browser and accepts command requests |
...
| Database (future) | **PostgreSQL** or **SQLite** via `pgx` / `go-sqlite3` | For persisting agent history, market snapshots, session replay data |
| Local LLM (future) | **HTTP client** to Ollama / vLLM / llama.cpp server | Go's `net/http` client calls the LLM's REST API; streaming responses handled via SSE or chunked transfer |
| Package manager | **npm** (frontend), **go modules** (backend) | Standard tooling for each |
...
type AgentConnection struct {
    ...
    attempts    int
    stateCh     chan GameMessage  // relay to browser clients
}

func NewAgentConnection(username, password string, stateCh chan GameMessage) *AgentConnection {
...
func (ac *AgentConnection) Run(ctx context.Context) {
    for {
        err := ac.connectAndListen(ctx)
        if ctx.Err() != nil {
            return // shutdown requested
        }
        log.Printf("[%s] disconnected: %v, reconnecting in %v", ac.username, err, ac.backoff)
        
        jitter := time.Duration(rand.Int63n(int64(1 * time.Second)))
        select {
        case <-time.After(ac.backoff + jitter):
        case <-ctx.Done():
            return
        }
        
        ac.backoff = min(ac.backoff*2, ac.maxBackoff)
        ac.attempts++
    }
}

func (ac *AgentConnection) connectAndListen(ctx context.Context) error {
    conn, _, err := websocket.DefaultDialer.DialContext(ctx, "wss://game.spacemolt.com/ws", nil)
    if err != nil {
    ...
    ac.backoff = 1 * time.Second  // reset on successful connect
    ac.attempts = 0

    // Read welcome, then login
    if err := ac.login(); err != nil {
        return fmt.Errorf("login: %w", err)
    }

    // Read loop: relay all messages to browser
    for {
...
All mutation commands flow through the Go backend, which sends them over the agent's WebSocket connection:

```
Browser → (localhost WS) → Go backend → (game server WS) → SpaceMolt

1. Browser sends JSON over localhost WebSocket:
   {"agent": "Atlas", "command": "travel", "payload": {"target_poi": "sys_0096_ice_field"}}

2. Go backend finds the agent's goroutine/connection, forwards to game server:
   {"type": "travel", "payload": {"target_poi": "sys_0096_ice_field"}}

3. Game server queues the action, executes on next tick (~10s), sends "ok" push.

4. Agent goroutine receives "ok", relays to all browser clients watching this agent.
```

```go
// Backend WebSocket handler for browser commands
func (s *Server) handleBrowserCommand(agentName string, cmd CommandRequest) {
    ...
    agent.ws.WriteMessage(websocket.TextMessage, msg)
}
```
...
### 9.2 Backend Agent Pool

```go
type Server struct {
    agents     map[string]*AgentConnection   // keyed by username
    browserWS  map[*websocket.Conn]string    // browser connections → which agent they're watching
    mu         sync.RWMutex
}

// Each agent runs in its own goroutine with independent reconnection
func (s *Server) AddAgent(username, password string) {
    stateCh := make(chan GameMessage, 100)
    agent := NewAgentConnection(username, password, stateCh)
    
    s.mu.Lock()
    s.agents[username] = agent
    s.mu.Unlock()
    
    go agent.Run(context.Background())       // game server connection
    go s.relayToBrowsers(username, stateCh)  // fan-out to browser clients
}
```
...
│   ├── go.mod
│   ├── go.sum
│   ├── main.go                # Entry point: HTTP server + WebSocket upgrade for browser clients
│   ├── agent.go               # Per-agent WebSocket connection to game.spacemolt.com
│   ├── reconnect.go           # Exponential backoff reconnection logic
│   ├── relay.go               # Fan-out game state to browser WebSocket clients
│   ├── commands.go            # Handle browser→game command proxying
│   ├── credentials.go         # Read/write agents.json (MVP credential store)
│   ├── cache.go               # Galaxy/recipe/ship class caching
│   ├── types.go               # SpaceMolt WebSocket message types
│   └── static.go              # Serve built frontend files (optional: embed via go:embed)
│
...
│   │   ├── hooks/
│   │   │   ├── useGameState.ts
│   │   │   ├── useWebSocket.ts    # Connect to Go backend's localhost WebSocket
│   │   │   └── useCommand.ts      # Send commands to backend via WebSocket
│   │   │
│   │   └── types/
...
│
└── ~/.spacemolt-observer/
    └── agents.json                # Credential store (not in repo)
```
...
- [ ] Set up project scaffolding: Go backend (`go mod init`), React frontend (Vite + TypeScript + Tailwind)
- [ ] Implement Go agent connection: WebSocket to `wss://game.spacemolt.com/ws`, login, message parsing
...
- [ ] Implement Go→browser WebSocket relay (localhost, fan-out game state to browser clients)
- [ ] Implement browser→Go command proxying (browser sends command, Go forwards to game server)
- [ ] Define Go structs for all SpaceMolt WebSocket message types
- [ ] Define TypeScript interfaces mirroring the Go types for frontend
